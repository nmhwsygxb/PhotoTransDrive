...
## 零服务器替代方案：第三方免注册转发（实测）

**不想租 VPS？** 用公开的免注册 TCP 转发服务，家里电脑主动连出即可，手机 App 用**直连模式**
填“服务商地址 + 端口 + 配对码”就能访问家里网盘（协议完全透明）。

### ✅ 推荐：bore.pub（免注册 TCP 隧道，实测稳定）

```bash
# 一键 (Windows: 双击 deploy/bore-up.bat)
python bore-up.py --pair-code 131420
```

脚本自动：启动 server.py → 找到/下载 bore 客户端（`bin/bore.exe`）→ 连上 bore.pub →
打印公网地址。看到 `✅ 公网地址就绪: bore.pub:XXXXX` 后，手机 App **直连模式**填：
...
实测：配对/认证/浏览/下载（sha256 与直连一致）/建目录/上传/删除，全功能经公网通过；
隧道连续运行 20+ 分钟稳定不断线。

### ⚠️ serveo.net（免注册 SSH 隧道）——实测不推荐

`serveo-up.py` 也可用（零依赖，用系统 ssh），但免费隧道**每约 4.6 分钟被服务端强制断开**
（规律 `Connection to serveo.net closed by remote host`, exit 255），且每次重连端口漂移——
**手机刚填好端口几分钟就失效**，实际不可用。保留脚本仅作参考/备选。

### 共同限制

- **端口每次启动/断线会变**：重新运行脚本，按窗口新打印的端口在手机重填
- **免费额度**：可能限速/限并发，家庭小流量可用，大流量请用自建 relay（本手册第一、二、三步）
- **安全性**：方案把 server.py 暴露到公网任意端口——**务必用强配对码**；
  直连模式建议用管理员码管理、普通码浏览（`--pair-code` 只读、`--admin-code` 完整权限）
